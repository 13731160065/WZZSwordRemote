...
{
    void(^_logMessageBlock)(NSString *);
}
...
- (void)logMessage:(void (^)(NSString *))aBlock {
    if (_logMessageBlock != aBlock) {
        _logMessageBlock = aBlock;
    }
}
...
- (BOOL)connectServerWithHost:(NSString *)host port:(NSString *)port {
    ...
    if (err) {
        if (_logMessageBlock) {
            _logMessageBlock([NSString stringWithFormat:@"%@", err]);
        }
        NSLog(@"%@", err);
        return NO;
    }
...
- (BOOL)onSocketWillConnect:(AsyncSocket *)sock {
    NSLog(@"将要连接c");
    if (_logMessageBlock) {
        _logMessageBlock([NSString stringWithFormat:@"将要连接服务端"]);
    }
    return YES;
}
...
- (void)onSocket:(AsyncSocket *)sock didConnectToHost:(NSString *)host port:(UInt16)port {
    if (_logMessageBlock) {
        _logMessageBlock([NSString stringWithFormat:@"已连接上服务端:%@:%d", host, port]);
    }
...
- (void)onSocket:(AsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag {
    if (_logMessageBlock) {
        _logMessageBlock([NSString stringWithFormat:@"读到数据:%@", [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding]]);
    }
